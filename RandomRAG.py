...
def performQuery(query, dataSet, retriever, args, modelName, model, tokenizer, token):
	...
	# select a random document from the knowledge-base
	data = dataSet.sample(n=1)
...
def main():
	...
	for idx,row in testSet.iterrows():
		...
		pred, retrievedMethod, retrievedSummary, prompt = performQuery(row['method'], dataSet, args, modelName, model, tokenizer, token)
